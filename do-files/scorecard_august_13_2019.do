...
(scatter hci_mf log_health_ed_pc if wbcountryname=="`ctry'" , mlabel(wbcountryname) mcolor(green) msymbol(solid) mlabcolor(green) mlabpos(9) mlabsize(medium)) ///
...
(scatter hci_mf log_health_ed_sp_pc if wbcountryname=="`ctry'" , mlabel(wbcountryname) mcolor(green) msymbol(solid)  mlabcolor(green) mlabpos(9) mlabsize(medium)) ///
...
(scatter drm lny  if drm<25 &  wbcountryname=="`ctry'" , mlabel(wbcountryname) mcolor(green)  msymbol(solid)  mlabcolor(green) mlabpos(9) mlabsize(medium)) ///
...
local date "05-28-2019"
	...
	foreach gender in mf {  //at this point only male+female (at later stage we can add m and f 
							...
							legend(off) title("{bf:Percentage that does not practice Open Defecation}", size(vlarge) pos(11)) xtitle("") ytitle("") yscale(range(0 2)) ylabel(none) xlabel(,labsize(large)) graphregion(fcolor(white)) 
							 ...
							 (scatter onesvec lasttfr if wbcountryname=="`ctry'" & year==2017 & lasttfr>0 & lasttfr<=`=scalar(r(r1))', msize(huge) msymbol(solid)  mlc(black) mfcolor(sky)) ///							 
                             (scatter onesvec lasttfr if wbcountryname=="`ctry'" & year==2017 & lasttfr>`=scalar(r(r1))' & lasttfr<=`=scalar(r(r2))', msize(huge) msymbol(solid) mlc(black)  mfcolor(gs10)) /// 							 
                             (scatter onesvec lasttfr if wbcountryname=="`ctry'" & year==2017 & lasttfr>`=scalar(r(r2))' & lasttfr<=10, msize(huge) msymbol(solid) mlc(black) mfcolor(sea)) /// 
		                      ,xline(`=scalar(r(r1))', lwidth(thick) lcolor(black) lpattern(solid) ) xline(`=scalar(r(r2))', lwidth(thick) lcolor(black) lpattern(solid))  ///
							legend(off) title("{bf:Total Fertility Rate}", size(vlarge) pos(11)) xtitle("") ytitle("") yscale(range(0 2)) ylabel(none) xlabel(,labsize(large)) graphregion(fcolor(white)) 
							 ...
							 (scatter onesvec lastafr if wbcountryname=="`ctry'" & year==2017 & lastafr>0 & lastafr<=`=scalar(r(r1))', msize(huge) msymbol(solid)  mlc(black) mfcolor(sky)) ///							 
                             (scatter onesvec lastafr if wbcountryname=="`ctry'" & year==2017 & lastafr>`=scalar(r(r1))' & lastafr<=`=scalar(r(r2))', msize(huge) msymbol(solid) mlc(black)  mfcolor(gs10)) /// 							 
                             (scatter onesvec lastafr if wbcountryname=="`ctry'" & year==2017 & lastafr>`=scalar(r(r2))' & lastafr<=200, msize(huge) msymbol(solid) mlc(black) mfcolor(sea)) /// 
		                      ,xline(`=scalar(r(r1))', lwidth(thick) lcolor(black) lpattern(solid) ) xline(`=scalar(r(r2))', lwidth(thick) lcolor(black) lpattern(solid))  ///
	...
	graph combine hci_`gender'_`ctry'.gph   lastspc_`ctry'.gph lastodcomp_`ctry'.gph, colfirst rows(2) cols(1) ysize(1) xsize(5) title("", suffix color(black) linegap(3)) graphregion(fcolor(white))  /// 
	note("", size (small))
	...
	"  green the third one." "- For fertility indicators, these colors are replaced with light blue," "gray and blue, respectively.", size (small))
